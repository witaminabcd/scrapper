...
function saveToExcel(data, workerId, fileName = `xlsx/products_${workerId}`) {
    try {
        // Create directory if it doesn't exist
        fs2.ensureDirSync('xlsx');
        ...
        if (fs2.existsSync(fileName)) {
            workSheets = xlsx.parse(fs2.readFileSync(fileName));
            // Keep existing data but ensure we have at least one sheet
        ...
        // Add new data to the first sheet
        workSheets[0].data.push(...data);
    ...
    while (true) {
        ...
        try {
            ...
            if (pageNum > 1 && pageNum % 3 === 0) {
                await log(workerId, `Processed 3 pages, waiting for 30 second...`);
                await new Promise(r => setTimeout(r, 30000)); // 1 минута задержки
            ...
            if (pageProducts.length > 0) {
                const txtFileName = `worker_${workerId}_page`;

                // const txtFileName = `worker_${workerId}_page_${pageNum}.txt`;
                await saveToExcel(pageProducts, workerId, txtFileName);
                await log(workerId, `Saved ${pageProducts.length} products from page ${pageNum} to ${txtFileName}`);
            }
